...
$outc="$file.pUNhet.cuantex.sigma3";
...
while($line=<FIL>) {
    ...
    foreach my $aa (sort keys %hall){
        ...
        ### balance btween het read counts
           ...
           $dp1=$dp[$hgen[0]];
            ...
            if($rel < $sigm1 or $rel >$sigm2){$cuhg++;$fu++;}
  ...
  #  print "unbalanced hets <$fu> total hets <$conhps>   pp <$pphu>\n"; <STDIN>;
   if($fu ne 0 and $conhps ne 0){$pphu=$fu/$conhps;}
    if($fu eq 0){$pphu=0;}
    if($conhps eq 0){$pphu=0;}
   ...
   if($pphu>0.6){$csu++;}
    ...
    if($fbad==1){
        ...
        foreach my $cc (@pbadg){
           ...
           #HERe  print "bad <$cc>  rawval in hid<$hreff{$cca}>  rawval<$hre{$cc}>           <$cbss> total<$gg>  fc<$fc>\n";
...
$pphu=$csu/$ccts;
print "proportion of het sites with all unbalanced het inds <$pphu>\n";
