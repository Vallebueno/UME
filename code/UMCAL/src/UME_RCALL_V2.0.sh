...
usage() { echo "Usage: $0 [-f <Merged.gvcf>] [-c <chromosome>] [-l <Phredscore cutoff>] [-x <clist | emp | Discovery | Production>]" 1>&2; exit 1; }
while getopts ":f:c:l:x:" o; do
    case "${o}" in
        x)
            x=${OPTARG}
            ((${x} == "clist" || ${x} == "emp" || ${x} == "Discovery" || ${x} == "Production")) || usage
            ;;
        f)
            f=${OPTARG}
            #((s == 45 || s == 90)) || usage
...
if [  ${x} == "emp" ] ; then
  ...
  echo "UME creating empirical db ....."
   #$srcDIR/Discovery/EmpDist2PvalV3.pl
...
if [  ${x} == "union" ] ; then
 ...
 echo "Done!!!"
  ...
  echo "UME UNION Discovery..."
   #perl $srcDIR/Discovery/DB_Union_V8_FAST.pl ${f} ${c} ${a} /groups/swarts/lab/MAVE/phred/Emp_Dist_CALLERS.pval2f ${q}
  echo "Done!!!"
