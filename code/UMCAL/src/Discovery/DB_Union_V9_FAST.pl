...
print $gout_fh "#0=missing info\n";
print $gout_fh "#1=reference allele\n";
print $gout_fh "#Mapped against B73-NAM AGPV5\n";
...
while($line=<DB>) {
  ...
  $ffof=0;
       ...
       if($fx==1){$win=$gan; print $gouu_fh "$hqx\n";}
        elsif($f==1){$win=$rfin;}
        elsif($fn==1){$win=0;}
        push(@winers, $win);
    }
