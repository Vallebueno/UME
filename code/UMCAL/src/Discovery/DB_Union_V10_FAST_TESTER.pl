...
if ($alg eq "sum") {
    ...
    $fsum = "/groups/swarts/lab/MAVE/phred/Emp_Dist_sum.txt.pval2f";
    ...
    foreach my $aa (@finh){
        ...
        foreach my $va (@check){
            ...
            if(exists $hpv{$caller}{$qual}){$norm=$hpv{$caller}{$qual};
                ...
                #print "xsxs <$aa> <$va> <$qual> <$norm> <$caller> ";<STDIN>;
            }
            else{
               ...
               # print "ysys <$aa> <$va> <$qual> <$norm> <$caller> <$bin> ";<STDIN>;
            }
            $ctr++;
            $h{$ll[0]}=$h{$ll[0]}+1;
            push(@{$posq{$ll[0]}}, $norm);
...
sub bubble_search {
    ...
    while ($low <= $high) {
        ...
        if ($array[$mid] == $var) {
            return $mid;  # Exact match found, return the index
        }
        if ($closest_index == -1 || abs($array[$mid] - $var) < abs($array[$closest_index] - $var)) {
            $closest_index = $mid;  # Update the closest index if closer value is found
        }
        if ($array[$mid] > $var) {
            $low = $mid + 1;  # Adjust the lower bound for descending order
        } else {
            $high = $mid - 1;  # Adjust the upper bound for descending order
        }
    }
