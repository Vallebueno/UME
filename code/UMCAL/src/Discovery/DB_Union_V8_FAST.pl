...
$out="$file.qual$qualf.Union.V8.$alg.dbx";
$out =~ s/.db.gz//g;
...
#open (my $gzip_fh, "| /bin/gzip -c > $out.gz") or die;

print OUT "#0=missing info\n";
print OUT "#1=reference allele\n";
print OUT "#Mapped against B73-NAM AGPV5\n";

#open (my $goou_fh, "| /bin/gzip -c > $out.q.V8.lstx.gz") or die;


#open(OUT,">$out") or die;
#open(OOQ,">$out.q.V7-1.lstx") or die;
