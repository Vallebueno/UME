...
geom_point(aes(color = dataF$V1), alpha=trsp)+
  geom_smooth(method = "lm", fill = NA, se=FALSE,formula = my.formula,size=2.5)+
...
text=legend
par(mfrow = c(1, 1), mar=c(3.5,3.5,0.1,0.1),bg=NA)
plot(x, y, xlab=paste("Coordinate ",i," (",round((fit$eig[i]/sum(fit$eig))*100,1),"%)",sep=""), ylab=paste("Coordinate ",j," (",round((fit$eig[j]/sum(fit$eig))*100,1),"%)",sep=""),main=main,col=color.inner,lwd=4,bg=paste(color.outer,"FF",sep=""),pch=21,cex=3.4,xlim=c(min(fit$points[,1]),max(fit$points[,1])+(max(fit$points[,1])-min(fit$points[,1]))*0),cex.axis=2,cex.lab=2, mgp = c(2, 0.5, 0))
legend(x = "bottomleft",legend = legend,col=color.inner.legend,pt.bg=color.outer.legend,pt.lwd = 2.5,pch=21,cex=1.5,pt.cex=1.7,text.width = strwidth(text)[1]*5.7,y.intersp=2)
#dev.off()
...
FIG1<-plot_grid(px,pmds,p1,p2,label_size = 30,labels = 'AUTO', hjust = 0, vjust = 1, scale= 0.97)

pdf("Vallebueno2022_Fig_1.pdf",width=6.75,height = 4.5)
FIG1
dev.off()
...
pdf("Vallebueno2022_Fig_1.pdf",width=25,height = 16)
FIG1
dev.off()
...
ggsave(file="Vallebueno2022_Fig_1.svg", plot=FIG1, width=25, height=16)
dev.off()
