...
while($line=<FIL>){
    ...
    if($info =~ /END=/){
        ...
        for($i=$pos;$i<=$END;$i++){
            ...
            if(exists $h{$key}){
                ...
                if($h{$key} !~ /$gref/){next;}
                if(exists $hall{$key}){print "WARN DDS1\n";<STDIN>};
                $hall{$key}="0/0:$mdp,0"; # print "<$key> <$hall{$key}> <$line>"; <STDIN>;
            }
        }
        next;
    } #end if SNR END
    if(exists $h{$key}) {
        ...
        if ($info =~ /DP=0/ ) {next;}
            ...
            @xbb=split(/;/,$xaa[1]);
            $mdp=$xbb[0];
            if(exists $hall{$key}){print "WARN DDS2\n";<STDIN>};
            $hall{$key}="0/0:$mdp,0"; # print "<$key> <$hall{$key}> <$line>"; <STDIN>;
            next;
        }
            ...
            @xbb=split(/;/,$xaa[1]);
            $mdp=$xbb[0];
            if(exists $hall{$key}){print "WARN DDS3\n";<STDIN>};
            $hall{$key}="0/0:$mdp,0"; # print "<$key> <$hall{$key}> <$line>"; <STDIN>;
            next;
        }
            ...
            @xbb=split(/;/,$xaa[1]);
            $mdp=$xbb[0];
            if(exists $hall{$key}){print "WARN DDS4\n";<STDIN>};
            $hall{$key}="0/0:$mdp,0"; # print "<$key> <$hall{$key}> <$line>"; <STDIN>;
            next;
        }
        #polimorphic
        ...
        if(scalar(@fin)==1){
            ...
            $ADAS[$val2]=$find[1];
            $sys=join(",", @ADAS);
            if(exists $hall{$key}){print "WARN DDSx\n";<STDIN>};

            if($val1<$val2){$hall{$key}="$val1/$val2:$sys"; next; } #print "a  <$key>  gbs<$h{$key}>  <$hall{$key}> <$line>"; <STDIN>;

            $hall{$key}="$val2/$val1:$sys"; # print "b  <$key>  gbs<$h{$key}> <$hall{$key}> <$line>"; <STDIN>;
            next;
        }
        ...
        $ADAS[$val2]=$find[$mms];
        $sys=join(",", @ADAS);
        if(exists $hall{$key}){print "WARN DDSy\n";<STDIN>};
        if($val1<$val2){$hall{$key}="$val1/$val2:$sys"; print "c <$key>  gbs<$h{$key}>  <$hall{$key}> <$line>"; <STDIN>;next;}

        $hall{$key}="$val2/$val1:$sys";  print "d <$key>  gbs<$h{$key}>  <$hall{$key}> <$line>"; <STDIN>;
        #####END polimorphic
    } ###end IF exists
} ### end FIL
close(FIL);
